...
Public Class TrxTeamLeader1
    ...
    Public Function TeamLeaderType(ByVal ID As String, ByVal UserName As String, ByVal GroupAgent As String, ByVal UserCreate As String, ByVal Action As String) As String

        If Action = "TABLE" Or Action = "SELECT" Or Action = "SEARCH" Or Action = "USERNAME" Or Action = "GROUPAGENT" Then
            Dim connstring As String = ConfigurationManager.ConnectionStrings("DefaultConnection").ConnectionString
            Dim dt As DataTable = New DataTable()
            Dim NameSP As String = "Exec SMG_TeamLeader_SP"
            Dim sql As String = "" & NameSP & " '" & ID & "', '" & UserName & "', '" & GroupAgent & "', '" & UserCreate & "', '" & Action & "'"
            Try
                Using conn As SqlConnection = New SqlConnection(connstring)
                    conn.Open()
                    Dim sqlComm As SqlCommand = New SqlCommand("SMG_TeamLeader_SP", conn)
        ...
        Else
            ...
            Try
                Using con As New SqlConnection(constr)
                    ...
                    sqlComm.CommandText = "SMG_TeamLeader_SP"
                    sqlComm.CommandType = CommandType.StoredProcedure
                    sqlComm.Parameters.AddWithValue("ID", ID)
                    sqlComm.Parameters.AddWithValue("UserName", UserName)
                    sqlComm.Parameters.AddWithValue("GroupAgent", GroupAgent)
            ...
            Catch ex As Exception
                ...
                strExec = "exec SMG_TeamLeader_SP '" & ID & "','" & UserName & "','" & GroupAgent & "','" & UserCreate & "','" & Action & "'"
