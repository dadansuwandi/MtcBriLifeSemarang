$(document).ready(function () {
    ...
    ComboReasonCall();
    $("#dialcall").hide()
    $("#Journey_ReasonCall").attr("disabled", true);
});
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for (i = 0; i < json.length; i++) {
                ...
                $("#ContentPlaceHolder1_TrxPhoneNumber").val(json[i].PHONE_PELAPOR)
                $('#Ticket_ReasonCall').append(json[i].channelid);
                $('#Ticket_EscalationType').append(json[i].NameType);
...
function post_WS_DataInteraction() {
    ...
    if ($("#Journey_Callback").val() == "No") {
        var TrxReasonCall = "-";
    } else {
        var TrxReasonCall = $("#Journey_ReasonCall").val();
    }
...
function ComboReasonCall() {
    var cmbDataReasonCall = $('#Journey_ReasonCall');
    $.ajax({
        type: "POST",
...
function CallbackChange(val) {
    var selectedText = $("#Journey_Callback").find("option:selected").text();
    var selectedValue = $("#Journey_Callback").val();
    if (selectedValue == "Yes") {
        $("#dialcall").show()
        $("#Journey_ReasonCall").attr("disabled", false);
    } else {
        $("#dialcall").hide()
        $("#Journey_ReasonCall").val("")
        $("#Journey_ReasonCall").attr("disabled", true);
    }
}
function ClickCall() {
    if ($("#ContentPlaceHolder1_TrxPhoneNumber").val() == "") {
        swal(
            '',
        ...
        swal({
            ...
            .then((willDelete) => {
                if (willDelete) {
                    ...
                    const url = "http://localhost:60024/dial/telp=" + $("#ContentPlaceHolder1_TrxPhoneNumber").val()
                    console.log("url" + url)
                    Http.open("GET", url);
                    Http.send();
    ...
    swal({
        ...
        .then((willDelete) => {
            if (willDelete) {
                ...
                const Http = new XMLHttpRequest();
                const url = "http://localhost:60024/drop/telp=" + $("#ContentPlaceHolder1_TrxPhoneNumber").val()
                Http.open("GET", url);
                Http.send();
