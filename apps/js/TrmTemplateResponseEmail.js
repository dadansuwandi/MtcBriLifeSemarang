    ...
    swal({
        ...
        .then((willDelete) => {
            if (willDelete) {
                ...
                var form_data = JSON.stringify({ ID: $("#ContentPlaceHolder1_TrxID").val(), Template: ResponseEmail, Format: $ESAPI.encoder().encodeForHTML($("#FormatType").val()), State: $("#cmbStatus").val(), User: $("#hd_sessionLogin").val(), Action: 'UPDATE' });
                $.ajax({
                    url: "asmx/TrmTemplateResponseEmail.asmx/AHU_Uidesk_TrxFormatResponseEmail_TSIUD",
                    method: "POST",
                    contentType: "application/json; charset=utf-8",
