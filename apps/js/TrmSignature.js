    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for (i = 0; i < json.length; i++) {
                ...
                myTable.row.add([json[i].rec_id, json[i].signature, json[i].account, urlClick]).draw(false);
            }
    ...
    swal({
        ...
        .then((willDelete) => {
            if (willDelete) {
                ...
                var form_data = JSON.stringify({ TrxID: $("#ContentPlaceHolder1_TrxID").val(), TrxUserName: $("#hd_sessionLogin").val(), TrxSignature: TrxSignature, TrxAccount: $("#ComboAccount").val()});
                $.ajax({
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for (i = 0; i < json.length; i++) {
                ...
                resultChannel = '<option value="' + json[i].incoming_account_name + '">' + json[i].incoming_account_name + '</option>';
                ComboAccount.append(resultChannel);
