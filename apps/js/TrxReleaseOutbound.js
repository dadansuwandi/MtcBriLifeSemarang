...
function ActionSubmit() {
    ...
    if ($("#ComboAction").val() == "Change") {
        ...
        if ($("#ChangeDate").val() == '' || $("#ChangeDate").val() == null) {
            swal(
                '',
                'Tanggal Data Outbound is empty',
                'info'
            ).then(function () {
                return false;
            });
        ...
        if ($("#ComboStatusData").val() == '' || $("#ComboStatusData").val() == null || $("#ComboStatusData").val() == "Select") {
            swal(
                '',
                'Status Data Outbound is empty',
                'info'
            ).then(function () {
                return false;
            });
            ...
            if (willDelete) {
                ...
                if ($("#ComboAction").val() == "Change") {
                    ...
                    $.ajax({
                        ...
                        data: "{ParameterID: '" + $("#ChangeDate").val() + "', UserName: '" + $("#hd_sessionLogin").val() + "', ParameterType: '" + $("#ComboStatusData").val() + "', ParameterValue: '" + $("#ComboAgent").val() + "', ParameterAction: '" + $("#ComboAction").val() + "'}",
                        contentType: "application/json; charset=utf-8",
                        dataType: "json",
                        success: function (data) {
                            ...
                            if (json.length > 0) {
                                for (i = 0; i < json.length; i++) {
                                    ...
                                    if (json[i].call_distribution_date == null) {
                                        var DateDistribute = "-"
                                    } else {
                                        ...
                                        var DateDistribute = newDate + ' ' + newTime
                                    }                                
                                    myTable.row.add([json[i].id, json[i].call_name, json[i].call_polis_number, json[i].call_phone_number, json[i].call_status, DateDistribute]).draw(false);
                    ...
                    $.ajax({
                        type: "POST",
                        url: "asmx/TrmUserManagement.asmx/UIDESK_TrmMasterCombo",
                        data: "{TrxID:'" + $("#TxtSearching").val() + "', TrxUserName: '" + $("#hd_sessionLogin").val() + "', TrxAction: 'UIDESK76'}",
                        contentType: "application/json; charset=utf-8",
                        dataType: "json",
                        success: function (data) {
                            var json = JSON.parse(data.d);
...
function ComboAgentOutbound() {
    ...
    $.ajax({
        ...
        data: "{TrxID:'0', TrxUserName: '" + $("#hd_sessionLogin").val() + "', TrxAction: 'UIDESK74'}",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (data) {
...
//function ComboStatusNya() {
//    var ComboCallStatus = $('#ComboCallStatus');
...
function ActionRelease() {
    if ($("#ContentPlaceHolder1_TrxOutboundID").val() == "") {
        swal(
            '',
            'Please check data outbound',
            'error'
        ).then(function () {
            location.href = "TrxReleaseOutbound.aspx";
        });
